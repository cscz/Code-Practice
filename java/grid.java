	...
	public static void main(String args[]) {
		...
		for(int i = 0; i < matrix.length; i++) {
			for(int j = 0; j < matrix.length; j++) {
				//int cal = cal(i, j, matrix);
				for(int m = i - 3; m <= i; m++) {
					if(m < 0) 
						continue;
					int k = 1;
					for(int n = 0; n < 4; n++) {
						if(m + n >= matrix.length)
							break;
						k *= matrix[m+n][j];	
					}
					ans = Math.max(ans, k);				
				...
				for(int m = 0; m < 4; m++) {
					...
					int k = 1;
					for(int n = 0; n < 4; n++) {
						if(m + n + j - 3 >= matrix.length || m + n + i - 3 >= matrix.length) 
							break;
						k *= matrix[m + n + i - 3][m + n + j - 3];
					}
					ans = Math.max(ans, k);				
				}
				for(int m = 0; m < 4; m++) {
					int k = 1;
					for(int n = 0; n < 4; n++) {
						if(i-m+n < 0 || j-n+m < 0 || i-m+n >= matrix.length || j-n+m >= matrix.length) 
							break;
						k *= matrix[i-m+n][j-n+m];
					}
					ans = Math.max(ans, k);				
				}
			}
